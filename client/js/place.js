...
var notificationHandler = {
    ...
    supportsNewNotificationAPI: false,
...
var place = {
    ...
    getRandomSpawnPoint: function () {
        ...
        return {
            x: getRandomTileNumber(),
            y: getRandomTileNumber()
        };
    },
    ...
    updateColourSelectorPosition: function () {
        var elem = $("#colour-picker-popover"),
            button = $("#customColourChooserOption");
        var position = 20;
    ...
    isOutsideOfBounds: function (precise = false) {
        var coord = this.getCoordinates();
        var x = coord.x < 0 || coord.x >= size,
            y = coord.y >= size || coord.y < 0
        return precise ? {
            x: x,
            y: y
        } : x || y;
    ...
    getCoordinates: function () {
        ...
        return {
            x: Math.floor(-this.panX) + dcanvas.width / 2,
            y: Math.floor(-this.panY) + dcanvas.height / 2
        };
    },
...
place.start($("canvas#place-canvas-draw")[0], $("#zoom-controller")[0], $("#camera-controller")[0], $("canvas#place-canvas")[0], $("#palette")[0], $("#coordinates")[0], $("#user-count")[0], $("#grid-hint")[0], $("#pixel-data-ctn")[0], $("#grid")[0], $("#message-dialog")[0]);
