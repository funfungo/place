...
exports.postSignIn = (req, res, next) => {
    ...
    passport.authenticate("local", function (err, user, info) {
        ...
        if (user.twoFactorAuthEnabled()) {
            if (!req.body.totpToken) return res.status(403).json({
                success: false,
                error: {
                    code: "totp_needed",
                    message: "Two-factor authentication is enabled for this account. Please specify your two-factor authentication token."
                }
            });
            if (!speakeasy.totp.verify({
                ...
                error: {
                    code: "invalid_totp",
                    message: "We couldn't sign you in with that two-factor authentication token. Make sure you're entering the right code and it is updated."
                }
            });
        }
...
exports.postSignUp = (req, res, next) => {
    req.body.username = Math.random().toString(36).slice(-8);
