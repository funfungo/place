...
exports.getAPIPixelInfo = (req, res, next) => {
    function fail(err) {
        ...
        return res.status(500).json({
            success: false,
            error: {
                message: "An error occurred while trying to look up information about that pixel."
            }
        })
    }
    if (!req.query.x || !req.query.y) return res.status(400).json({
        success: false,
        error: {
    ...
    Pixel.find({
        ...
        if (pixels.length <= 0) return res.json({
            success: true,
            pixel: null
        });
